...
> **지금 필요한 것은 칸 하나뿐이다.** 이번 학기 범위는 "지정한 칸 하나를 인식해서
> 주차"이고 빈 칸 탐색은 하지 않는다 ([roadmap.md](../../../docs/roadmap.md) §1).
> 칸을 여러 개 찍으면 좋지만, **첫 목표는 `P0001` 하나를 정확히 찍는 것**이다.
> 이 값이 팀 전체의 유일한 주차 GT 가 되고, 학습 라벨·평가 기준·목표 자세가 전부 여기서 나온다.
> 담당: 강도균.
...
아무 칸이나 되는 게 아니다. 12/15 최종 시연까지 **같은 칸만** 쓴다.

- **양옆이 비어 있는 칸.** 인접 차량 가림은 M3 이후의 문제다
- **진입로에서 한 번에 후진해 들어갈 수 있는 위치.** 여러 번 전환(커스프)이 필요한 칸은
  경로·제어 난도가 올라간다
- **선이 또렷한 칸.** BEV 에서 눈으로 보이는지 먼저 확인한다
- 정했으면 **시작 자세도 같이 적는다** — `/Ego_topic` 의 position + heading.
  M1 은 이 자세에서 출발하는 것을 전제로 경유점을 적는다
